...
//   switch (kc) {
//     case KC_LCTL:
//       state.mods.CTRL = state.mods.CTRL_L = active;
//       break;
//     case KC_RCTL:
//       state.mods.CTRL = state.mods.CTRL_R = active;
//       break;
//     case KC_LGUI:
//       state.mods.GUI = state.mods.GUI_L = active;
//       break;
//     case KC_RGUI:
//       state.mods.GUI = state.mods.GUI_R = active;
//       break;
//     case KC_LALT:
//       state.mods.ALT = state.mods.ALT_L = active;
//       break;
//     case KC_RALT:
//       state.mods.ALT = state.mods.ALT_R = active;
//       break;
//     case KC_LSFT:
//       state.mods.SHIFT = state.mods.SHIFT_L = active;
//       break;
//     case KC_RSFT:
//       state.mods.SHIFT = state.mods.SHIFT_R = active;
//       break;
//   }
// }
...
// process_stats();
// process_hype();
#ifdef CORE_FEATURE_ENABLED
  process_core_feature(kc, rec);
#endif
#ifdef COMBO_FEATURE_ENABLED
  process_combo_feature(kc, rec);
#endif
#ifdef MACROS_FEATURE_ENABLED
  process_macro_feature(kc, rec);
#endif
#ifdef OS_FEATURE_ENABLED
  process_os_feature(kc, rec);
#endif

#ifdef REMAP_FEATURE_ENABLED
  process_remap_feature(kc, rec);
#endif
}

bool process_record_user(uint16_t kc, keyrecord_t *rec) {
#ifdef CORE_FEATURE_ENABLED
  handle_core_feature(kc, rec);
#endif

#ifdef COMBO_FEATURE_ENABLED
  handle_combo_feature(kc, rec);
#endif
#ifdef MACROS_FEATURE_ENABLED
  handle_macro_feature(kc, rec);
#endif
#ifdef OS_FEATURE_ENABLED
  handle_os_feature(kc, rec);
#endif

#ifdef REMAP_FEATURE_ENABLED
  handle_remap_feature(kc, rec);
#endif
